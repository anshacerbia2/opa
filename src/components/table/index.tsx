...
import { IAgentResponse } from "../../redux/apis/agents/agents.api";
...
const Table = ({
  ...
  useEffect(() => {
    // console.log("table:", dataPrefix, error, isLoading, data);
    ...
    <div className="c-table-wrapper">
      <div className="c-table-utility">
        <button type="button" className="c-btn">
          <IoPersonAdd className="c-icon" /> Create New
        </button>
        <button type="button" className="c-btn">
          <SiMicrosoftexcel className="c-icon" /> Export
        </button>
      </div>
      <div className="table-responsive mb2">
        <table className="table table-bordered">
          ...
          <tbody>
            {error ? (
              <tr>
                <td colSpan={dataPrefix.length}>Oh no, there was an error</td>
              </tr>
            ) : isLoading ? (
              <tr>
                <td colSpan={dataPrefix.length}>Loading...</td>
              </tr>
            ) : datas && datas.data.length ? (
              datas.data.map((dataRow: any, rowIndex: number) => {
                return (
                  <tr key={`data-row-${rowIndex}`}>
                    {dataPrefix.map((dataCol, colIndex) => {
                      return (
                    ...
                    <td>
                      <div className="c-table-action-wrapper">
                        <button className="c-btn" title="Edit">
                          <svg
                            viewBox="0 0 24 24"
                            fill="none"
